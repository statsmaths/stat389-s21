...
We can understand these coefficients best by plotting them over the
course of the year:

```{r}
tibble(month = names(coef(model)[-1]), val = coef(model)[-1]) %>%
...
```{r}
tibble(month = rownames(coef(model))[-1], val = coef(model)[-1]) %>%
  ggplot(aes(factor(month, levels = tolower(month.abb)), val)) +
    geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
    geom_path(aes(group = 1), size = 1) +
    geom_point(size = 3) +
...
some coefficients to zero but tries to spread the weight out evenly
over higher correlated groups of variables. Here's an example on our
data set:
...
a true blend between the ridge and lass models

### Model selection

Let's look at the California house price data set again.
...
mf <- model.frame(median_house_value ~ . -1,
                  data = select(ca, -train_id))
mt <- attr(mf, "terms")
y <- model.response(mf)
X <- model.matrix(mt, mf)
...
Unlike with `glm` we specify the name in quotes. We will use a "multinomial"
model here so that our code can be used verbatim in situations with more than
two categories.
...
Here, we see the most important variables in the model for a specific value
of lambda
...
will include different information in the case of 3 or more categories.

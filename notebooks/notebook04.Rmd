...
```{r}
model <- cv.glmnet(X_train, y_train, alpha = 0.9, family = "binomial")
```
...
So, shot distance seems to be the most important, followed
...
```{r}
mat <- coef(model, s = c(model$lambda.min, model$lambda.1se))
mat[mat[,1] != 0, ]
```

Often, the best model on new data sets will be the model set with the
lambda with a cross-validation lose 1 standard error greater than the minimum

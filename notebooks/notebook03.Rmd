...
model <- nba %>%
  filter(train_id == "train") %>%
  lm(fgm ~ shot_clock + close_def_dist, data = .)

nba %>%
  mutate(fgm_pred = predict(model, newdata = .)) %>%
  mutate(fgm_pred_bin = (fgm_pred > 0.5)) %>%
  ggplot(aes(shot_clock, close_def_dist)) +
    geom_point(aes(color = fgm_pred_bin)) +
    scale_color_viridis_d()
```
...
model <- nba %>%
  filter(train_id == "train") %>%
  lm(fgm ~ shot_clock + defender_height, data = .)

nba %>%
  mutate(fgm_pred = predict(model, newdata = .)) %>%
  mutate(fgm_pred_bin = (fgm_pred > 0.5)) %>%
  ggplot(aes(shot_clock, defender_height)) +
    geom_point(aes(color = fgm_pred_bin)) +
    scale_color_viridis_d()
```

And again, the pattern matches the relationship we would expect and
shows how the two regions are separated by a straight line.

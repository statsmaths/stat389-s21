...
while also introducing the form of the third project.
...
even predict the gender of the reviewer or business category. And yes, in fact
you are welcome to do all of these things. But today I want to show another
...
of each of these meta-documents. In the notes, I will work with the business
categories. For the project, you will also look at the other variables.
...
`biz_category` in place of `doc_id` to create a matrix with just one row per
...
$$ W_1 = \text{argmax}_{\; v : || v ||_2 = 1} \left\{ v^t X^t X v \right\} $$
